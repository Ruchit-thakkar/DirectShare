  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-4 animate-fade-in">
      ...
      {filteredLogs.length === 0 ? (
        <div className="border border-dashed border-white/10 rounded-3xl p-12 text-center bg-slate-800/10 animate-fade-in">
        ...
        <div className="grid grid-cols-1 gap-3.5 animate-fade-in">
          {filteredLogs.map((log) => {
            const { icon: Icon, colorClass, category } = getFileTypeVisualsByFileName(log.fileName, log.fileType);
            return (
              <div
                key={log.id}
                className="glass-panel p-4 rounded-xl border border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-white/20 transition-all hover:bg-white/[0.02]"
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0 space-y-0.5">
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-500">
                      <span className="font-mono">{formatBytes(log.fileSize)}</span>
                      <span>•</span>
                      <span className="text-primary/70 font-semibold uppercase tracking-wider text-[10px]">{category}</span>
                      <span>•</span>
                ...
                <div className="flex items-center gap-3.5 sm:text-right shrink-0 border-t sm:border-t-0 border-white/5 pt-3 sm:pt-0">
                  <div className="space-y-0.5 text-left sm:text-right">
                    ...
                    {log.speed > 0 && (
                      <p className="text-xs text-slate-400 flex items-center sm:justify-end gap-1 font-mono">
