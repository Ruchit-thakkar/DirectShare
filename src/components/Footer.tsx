...
export default function Footer() {
  ...
  const socialLinks = [
    ...
    { 
      ...
      icon: GithubIcon, 
      color: 'hover:text-slate-200 hover:bg-white/10 hover:border-white/20' 
    },
    { 
      name: 'X', 
    ...
    { 
      ...
      icon: Mail, 
      color: 'hover:text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-500/20' 
    },
  ];

  return (
    <footer className="w-full border-t border-white/5 py-10 bg-slate-950/40 text-slate-500 mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-300 text-lg">DirectShare</span>
          </Link>
          ...
          <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2.5">
            {navLinks.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-sm text-slate-400 hover:text-slate-200 transition-colors duration-250"
              >
                {item.name}
              </Link>
            ))}
            ...
            {socialLinks.map((social) => {
              ...
              return (
                <a
                  ...
                  title={social.name}
                  className={`p-2 rounded-xl bg-slate-900 border border-white/5 text-slate-400 transition-all duration-300 hover:scale-110 ${social.color}`}
                >
